...
def create_final_catalog():
    ...
    try:
        ...
        row_cells[0].width = Inches(0.5)  # 与序号列等宽
        
        # B1:H1: 档号内容 (真正合并)
        for i in range(1, 8):
            row_cells[i]._element.getparent().remove(row_cells[i]._element)
        row_cells[1]._element.getparent().append(row_cells[1]._element)
        ...
        merged_cell.paragraphs[0].paragraph_format.left_indent = Inches(0.2)
        ...
        row_cells[4].width = Inches(0.7)  # 与日期列等宽
        
        # F3:H3: 客户代码值 (合并)
        merged_cell = row_cells[5].merge(row_cells[7])
        merged_cell.text = client_code
        merged_cell.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.LEFT
        merged_cell.paragraphs[0].paragraph_format.left_indent = Inches(0.2)
        ...
        # 保存文件
        output_path = r"D:\\申银万国期货\\3.档案管理\\2、业务档案\\2. 未归档项目-2025\\毛喜瑞交易咨询项目\\卷内文件目录_最终终极完美版.docx"
        doc.save(output_path)
        print(f"✅ 最终终极完美版已成功生成: {output_path}")
